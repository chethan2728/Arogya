...
const SpecialityMenu = () => {
  return (
    <div className='flex flex-col items-center gap-4 py-16 surface-text' id='speciality'>
      ...
      <div className='flex sm:justify-center gap-4 pt-5 w-full overflow-scroll scene-3d'>
        {specialityData.map((item,index) => (
            <Link onClick={()=>scrollTo(0,0)} className='flex flex-col items-center text-xs cursor-pointer flex-shrink-0 transition-all duration-500 glass-card px-4 py-5 rounded-2xl min-w-[120px] tilt-card' key={index} to={`/doctors/${item.speciality}`}>
