    ...
    <div>
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          <div className='flex items-center gap-2 mt-1 text-gray-600'><p>{profileData.speciality} - {profileData.degree}</p>
          ...
          <div className='flex gap-2 py-2'>
            <p>Address:</p>
            <p className='text-sm'>
              {isEdit ? <input className='border px-1' type="text" onChange={(e)=>setProfileData(prev => ({...prev, address: {...prev.address, line1: e.target.value}}))} value={profileData.address.line1} /> : profileData.address.line1},
              <br />
              {isEdit ? <input className='border px-1' type="text" onChange={(e)=>setProfileData(prev => ({...prev, address: {...prev.address, line2: e.target.value}}))} value={profileData.address.line2} /> : profileData.address.line2}
            </p>
          </div>
          ...
          {isEdit 
          ? <button onClick={updateProfile} className='px-4 py-1 bg-primary text-white text-sm rounded-full mt-5 hover:bg-primary/90 transition-all'>Save</button> 
