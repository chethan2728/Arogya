    ...
    <div className='m-5 soft-text'>
      <div className='flex flex-wrap gap-3'>
        <div className='flex items-center gap-2 glass-card p-4 min-w-52 rounded-2xl cursor-pointer hover:scale-105 transition-all'>
          ...
          <div>
            <p className='text-xl font-semibold surface-text'>{currency} {dashData.earnings}</p>
            <p className='soft-text'>Earnings</p>
          </div>
        </div>
        ...
        <div className='pt-4'>
          {
            dashData.latestAppointments.map((item, index) => (
              <div className='flex items-center px-6 py-3 gap-3 hover:bg-cyan-300/5' key={index}>
                <img className='rounded-full w-10 bg-sky-900/40' src={item.userData.image} alt="" />
                <div className='flex-1 text-sm'>
                  <p className='surface-text font-medium'>{item.userData.name}</p>
